...
def _euler_to_positive_w_quat(euler_xyz):
    ...
    decode as +sqrt(1 - x^2 - y^2 - z^2) (see LMTKeyFrames.restore_w) - i.e.
    it only round-trips quaternions whose w is already non-negative. q and
    -q represent the same rotation, so negating here to force w >= 0 loses
    nothing and matches how genuine game files are authored.
    """
...
def local_game_fs(pytestconfig, local_app_id):
    ...
    if not value:
        ...
        pytest.skip(
            "test_lmt_reimport_roundtrip needs a local game root - ArcFS opens a plain "
...
def test_reimport_through_import_operator(
    tmp_path, local_game_fs, local_app_id, local_mod_path_hash, local_lmt_path_hash
):
    ...
    vfs.add_fs_root(app_id, mod_arc_fs, display_name="roundtrip-mod")
    ...
    vfs.add_fs_root(app_id, lmt_arc_fs, display_name="roundtrip-lmt")
    ...
    assert action_a and action_a.fcurves

    # load_lmt() only auto-wires animation_data.action for Blender 5+ - on
    # 4.x it stops at animation_data_create(), so applying/editing the pose
    # requires this assignment done by hand (see test_lmt_single_arc_import.py).
    armature_a.animation_data_create()
    armature_a.animation_data.action = action_a
    bpy.context.scene.frame_set(1)

    bone_names_with_rotation = {
        fc.data_path.split('"')[1] for fc in action_a.fcurves
    ...
    vfs.add_fs_root(app_id, reimport_fs, display_name="roundtrip-reimport-lmt")
    ...
    assert action_b and action_b.fcurves
