...
try:
    ...
    from vecOp import vectInt, vecMult, vecDiv, vecUnfold
except:
...
class BoundingBox():
    ...
    def intersect(self,bb):
        minima = Vector([max(w0,w1) for w0,w1 in zip(self.minPos,bb.minPos)])
        maxima = Vector([min(w0,w1) for w0,w1 in zip(self.maxPos,bb.maxPos)])
    ...
    def serialize(self):
        return {"minPos":vecUnfold(self.minPos),"maxPos":vecUnfold(self.maxPos)}
    ...
    def encode(self):
        if self.encodable:
            if hasattr(self,"mortonCode"):
                ...
                normalized = vecDiv((self.barycenter()-self.sceneStart),self.sceneSize)
                self.mortonCode = mortonEncode(*(vectInt(mortonLimit*normalized)))
                return self.mortonCode
        else:
...
class Tri(GeometryPrimitive):
    def __init__(self,triface,vertList):
        ...
        self.vertices = [Vector(decontainer(vertList[triface.vert[i]])) for i in range(3)]
    ...
    def barycenter(self):
        if not hasattr(self,"_barycenter"):
            self._barycenter = sum(self.vertices[1:],self.vertices[0])/len(self.vertices)
    ...
    def normal(self):
        if not hasattr(self,"normalVector"):
            self.normalVector = Vector(np.cross(self.vertices[1]-self.vertices[0],self.vertices[2]-self.vertices[0]))
    ...
    def mergeable(self, tri2):
        if type(self) != Tri or type(tri2) != Tri:
            return False
...
class QuadPair(Tri):
    ...
    def boundingBox(self):
        if not hasattr(self,"_boundingBox"):
            self._boundingBox = self.dataFaces[0].boundingBox().merge(self.dataFaces[1].boundingBox())
        return self._boundingBox
    def quad(self,tri2):
...
# =============================================================================
    ...
    def recursiveGraph(tree,graph,tid):
        for children in tree.children():
            cid = genUniqueId()
            graph.add_node(cid, label = children.index(),shape='box' ,style = 'filled',color = typeMap[children.typePair()])
    ...
    def plotQTree(tree,sbcName=Path("test")):
        ...
        tid = genUniqueId()
        g.add_node(tid, label = tree.index(),shape='box',style = 'filled',color = typeMap[tree.typePair()])
        graph = recursiveGraph(tree,g,tid)
        
        graph.layout(prog='dot')
        graph.draw(str(Path(r"E:\MapModding\AggloHierarchy").joinpath(sbcName.stem + '.pdf')))
    
    algo = lambda x: bvc.spatialSplits(x, partition = bvc.linearSplit, metric = bvc.Cluster.SAH_EPOMetric)
    ...
    superTree = next(iter(algo([tree.setIndex(ix)  for ix,tree in enumerate(trees)]))).collapse()
    superTree.indexize()
