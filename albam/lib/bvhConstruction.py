...
try:
    from primitiveGeometry import mortonLength, QuadPair, PrimitiveTree, BoundingBox
    from lowLevelOperators import radixSort
except:
    from .primitiveGeometry import mortonLength, QuadPair, PrimitiveTree, BoundingBox
    from .lowLevelOperators import radixSort
...
class Cluster():
    ...
    def __init__(self, element):
        ...
        if not isIterable(element):
            ...
            while True:
                try: self.aabb = self.aabb.merge(next(elements).boundingBox())
                except:break
            self.count = sum([i.count if hasattr(i,"count") else 1 for i in element])
        self.closest = None
    def isNode(self):
    ...
    def tabbedString(self,level=0):
        ...
        children = "".join((i.tabbedString(level+1) for i in self.content if i)) if self.isNode() else ""
        return selfstring+children
                    
    @staticmethod
    def SAHMetric(s1,s2):
        s = Cluster((s1,s2))
        ratio = lambda x: x.surfaceArea()/s.surfaceArea()*x.count 
        T = ratio(s1)+ratio(s2)
...
class QBVH():
    def __init__(self, binaryCluster, ll=None,lr=None,rl=None,rr=None):
        ...
        if binaryCluster is None:
            ...
            if not self.isPrimitive():
                check = lambda x: QBVH(None) if x is None else x
                self.ll = check(ll)
                self.lr = check(lr)
                self.rl = check(rl)
                self.rr = check(rr)
    ...
    def typePair(self):
        matrix = {Cluster.EMPTY:(0,0),Cluster.PRIMITIVE:(1,0),Cluster.NODE:(0,1)}
    ...
    def typeMask(self):
        if not self.isNode():
            raise NotImplementedError("Empties and Primitives don't have a type mask.")
        else:
            primitive, node = list(map(list,map(reversed,zip(*[subnode.typePair() for subnode in [self.ll,self.lr,self.rl,self.rr]]))))
            lshiftOr = lambda x,y: (x<<1)|y
            return [reduce(lshiftOr,primitive+node)]*4
    def nodeId(self):
        if not self.isNode():
            raise NotImplementedError("Empties and Primitives don't have subnodes.")
        else:
    ...
    def childBoxes(self):
        boxes = []
        default = self.parent.childBoxes() if self.parent else [self.node.boundingBox()]*4
        for ix,child in enumerate(self.children()):
            boxes.append(child.AABB() if child.isNode() or child.isPrimitive() else default[ix])
        return boxes
    def extremaAABB(self, bbFunctor):
        boxes = self.childBoxes()
        intersped = lambda w: [bbFunctor(entry)[w] for entry in boxes]
    ...
    def separateTraverse(self):
        if not hasattr(self,"traversalBuffer"):
            if self.isNode():
                ...
                self.traversalBuffer = ([self]+sum([c[0] for c in childElements],[]),sum([c[1] for c in childElements],[]))
            ...
            for cj in clusters:
                if ci != cj:
                    ...
                    if  m < best:
                        ...
                        left = ci
                        right = cj
        setRemove(setRemove(clusters,left),right).add(BinaryCluster((left,right)))
    return clusters
...
def buildTree(primitives,metric,countReduction,threshold):
    ...
    build = lambda x: buildTree(x,metric,countReduction,threshold)
...
def mortonPartition(encodableList, metric = None, mortonPoint=0):
    ...
    key = lambda x: ((2<<bit)&encodableList[x].encode())#>>32
    left = 0
...
def _kdTreeSplit(indices,access,axis,mode):
    ...
    for ax,dim in enumerate(indices):
        if ax == axis:
            ...
            l,r = [ix for ix in dim if ix in lset],[ix for ix in dim if ix not in lset]
            lindices.append(l)
            rindices.append(r)
...
def mortonSort(primitives):
    unpack = lambda x: [x.minPos, x.maxPos]
    mergeOp = lambda x, y: x.merge(y)
...
def _spatialSplits(clusters, metric = Cluster.SAH_EPOMetric, partition = linearSplit, **kwargs):
    ...
    if l and r:
        lcluster, rcluster = _spatialSplits(l,metric,partition),_spatialSplits(r,metric,partition)
...
# =============================================================================
def indexizeObject(listing, key=None):
