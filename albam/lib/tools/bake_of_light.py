...
def _bake_light(selected_objects, lm_size, app_id):
    lm_ob = {}
    vc_ob = []
    baked_img_cache = set()
    lm_mats = []
    for ob in selected_objects:
        ...
        if custom_properties.func_lightmap in ("0x5", "0x6"):
            ...
            if not parent:
                continue
            assert len(ob.data.uv_layers) >= 2, f"{ob.name} doesn't have lightmap uv layer"
            if has_nmap and len(ob.data.uv_layers) == 2:
                _create_uv_for_normal_map(ob)
            _rename_uv_layers(ob)
            if lm_ob.get(parent.name):
                lm_ob[parent.name].append(ob)
            else:
                lm_ob[parent.name] = [ob]
...
def _get_albam_mat_props(ob, app_id):
    bl_mat = ob.data.materials[0]
    try:
        custom_properties = bl_mat.albam_custom_properties.get_custom_properties_for_appid(app_id)
    except AttributeError:
        print(f"That isn't albam material:{ob.name}")
...
def _setup_mtfw_material(bl_mat, lmap_name='Lightmap LM', size=2048):
