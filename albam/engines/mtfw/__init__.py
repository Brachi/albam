...
# Devil May Cry 4's file ids are a different hash of the same resource class
# names the ids above hash, so its archives need their own table. What each id
# is was read off the data rather than computed: every entry of a given id
# decompresses to the same magic across the committed dataset, the magic names
# a resource class the game binary carries a string for, and the class name
# hashes (through the hash the ids above use) to the extension already listed
# there. The ids left out are the ones whose payload is a generic container
# (XFS) or plain text, which names no class on its own.
FILE_ID_TO_EXTENSION_DMC4 = {
    0x07D5909F: "stq",   # rSoundStreamRequest
    0x094973CF: "sca",   # rSoundCurveSet
